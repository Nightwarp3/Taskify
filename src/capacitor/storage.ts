/**
 * Capacitor storage layer — async port of src/main/db.ts.
 * Uses @capacitor/preferences as the backing store instead of electron-store.
...
async function load<T>(key: string, fallback: T): Promise<T> {
  const { value } = await Preferences.get({ key: `taskify:${key}` })
  if (!value) return fallback
  try { return JSON.parse(value) as T } catch { return fallback }
}

async function save(key: string, value: unknown): Promise<void> {
  await Preferences.set({ key: `taskify:${key}`, value: JSON.stringify(value) })
}
